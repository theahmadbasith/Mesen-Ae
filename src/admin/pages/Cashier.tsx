...
export default function Kasir() {
  ...
  const processCheckoutToDb = async (checkoutData?: {
    finalPayments: any[];
    ...
    try {
      ...
      let primaryMethodId = 0;
      let finalTax = taxAndService;
      let finalTotal = total;
      let finalChange = change;
      let finalCust = customerName.trim();
      let finalTable = tableNumber.trim();
      let finalRemarks = remarks.trim();

      if (checkoutData) {
        finalPayments = checkoutData.finalPayments;
        primaryMethodId = checkoutData.primaryMethodId;
        finalTax = checkoutData.taxAndService;
        finalTotal = checkoutData.total;
        finalChange = checkoutData.change;
        finalCust = checkoutData.customerName;
        finalTable = checkoutData.tableNumber;
        finalRemarks = checkoutData.remarks;
      } else {
        const currentMethod = paymentMethods?.find(m => m.id!.toString() === paymentMethodId);
        const finalPaymentsArr = [...payments];
        if (currentPaidAmount > 0 && currentMethod) {
          finalPaymentsArr.push({
        ...
        finalPayments = finalPaymentsArr;
        primaryMethodId = finalPaymentsArr.length > 0 ? finalPaymentsArr[finalPaymentsArr.length - 1].methodId : 0;
      }
