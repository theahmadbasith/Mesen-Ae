...
export interface Banner { id?: string | number; type?: string; title: string; description?: string; voucherId?: number | null; productId?: number | null; imageUrl?: string | null; buttonText?: string; link?: string; isActive: boolean; bgType?: 'image' | 'solid' | 'gradient'; bgColor?: string; bgGradient?: string; canvasLayers?: any[]; canvasBgFilter?: any; createdAt?: string; descPos?: { x: number, y: number }; overlayPos?: { x: number, y: number }; buttonPos?: { x: number, y: number }; }
