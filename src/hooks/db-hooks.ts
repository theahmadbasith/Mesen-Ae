...
export interface Banner { id?: string | number; title: string; description?: string; imageUrl: string; isActive: boolean; link?: string; overlayImageUrl?: string | null; titlePos?: { x: number, y: number }; descPos?: { x: number, y: number }; overlayPos?: { x: number, y: number }; buttonPos?: { x: number, y: number }; buttonText?: string; }
